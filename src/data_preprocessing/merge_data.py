...
def filter_outliers(df, column, low_multiplier=1.5, high_multiplier=4.0):
...
def merge_data(market_data: dict, card_attributes: pd.DataFrame) -> pd.DataFrame:
    """Merge market_data and card_attributes dataframes."""
    try:
        ...
        raise# src/data_processing/merge_data.py
import pandas as pd
import numpy as np
from src.utils.logger import logger
from src.utils.error_handler import log_error
...
def filter_outliers(df, column, low_multiplier=1.5, high_multiplier=4.0):
    ...
    logger.info(f"Filtered {stats['removed_count']} outliers from {column} (low_multiplier: {low_multiplier}, high_multiplier: {high_multiplier})")
    return filtered, stats
...
def merge_data(market_data: dict, card_attributes: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        # Filter outliers from sales_history.price
        sales_history_filtered, outlier_stats = filter_outliers(sales_history, "price", low_multiplier=1.5, high_multiplier=4.0)
